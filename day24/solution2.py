...
# current state as (time, position, goal, iteration)
state = (0, start, end, 0)
to_evaluate = [state]
...
while to_evaluate:
    ...
    if pos == goal:
        ...
        to_evaluate.clear()
        _states.clear()
        name = "end" if goal == end else "start"
        print(f'Iteration {it} ... Reached {name} position {pos} at time: {t}')
        if it == 1:
            to_evaluate.append((t, pos, start, it))
        elif it == 2:
            to_evaluate.append((t, pos, end, it))
        elif it == 3:
            break
    else:
        ...
        if pos not in blizzards[t+1]:
            to_evaluate.append((t+1, pos, goal, it))
        ...
        for d in dydx:
            ...
            if nx in range(0, x_max+1) and ny in range(0, y_max+1) and not is_wall(n) and n not in blizzards[t+1]:
                to_evaluate.append((t+1, n, goal, it))
